...
# Hour (UTC) to post the daily fixture preview — one compiled post
# covering every watched-team fixture kicking off that whole day
# (built fresh from team_fixtures.get_today_schedule() at post time,
# not from whatever happens to be live at this hour).
DAILY_PREVIEW_HOUR  = int(os.getenv("DAILY_PREVIEW_HOUR", "7"))

# ── Goal confirmation delay ──────────────────────────────────────────
# A newly-seen goal must survive being seen again GOAL_CONFIRM_SECONDS
# later before it's allowed to become a live post — this is the
# primary defence against ever posting a goal SofaScore's own VAR
# review is about to overturn. Project policy: being a poll cycle late
# is fine; posting something false is not. Default (120s = 2x the
# default 60s POLL_INTERVAL) means a goal must still be present on at
# least the NEXT poll tick, not just the one it was first seen on —
# comfortably covers most real-world VAR review times. Only gates the
# LIVE card: half-time/full-time scorer lines read match["goals"]
# directly and are never delayed by this.
GOAL_CONFIRM_SECONDS = int(os.getenv("GOAL_CONFIRM_SECONDS", "120"))
...
LINEUP_LEAD_MINUTES  = int(os.getenv("LINEUP_LEAD_MINUTES", "55"))
...
#
# Every league is switched purely by its LEAGUE_* env var — set these
# directly in Railway's Variables tab. No file to edit, no redeploy
# lag: Railway restarts the worker as soon as a variable changes, and
# the new value takes effect on the very next tick.
...
WATCHED_LEAGUES = {
    ...
    "Liga MX":                   _league_on("LEAGUE_LIGA_MX"),
    "Championship":              _league_on("LEAGUE_CHAMPIONSHIP"),
}
...
# ── Favorites (busy-day filter) ───────────────────────────────────────
# FAVORITES=on/true -> within a league that's still ON in WATCHED_LEAGUES
# above, ONLY matches involving a club listed in data/favorites.json get
# posted (the other, non-favourite clubs in that league are skipped that
# day). A league already switched OFF in WATCHED_LEAGUES stays fully off
# regardless of this flag — league toggle is always checked first,
# favourites only narrows further within a league that's already ON. If
# a favourite team is playing a non-favourite team, the match still
# posts (only ONE side needs to be a favourite). FAVORITES=off (default)
# changes nothing — behaves exactly as before this flag existed.
FAVORITES_MODE = os.getenv("FAVORITES", "off").strip().lower() in ("on", "true", "1", "yes")


def _load_favorite_team_ids() -> set[str]:
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "favorites.json")
    try:
        with open(path, encoding="utf-8") as f:
            by_league = json.load(f)
        return {str(team_id) for teams in by_league.values() for team_id in teams.keys()}
    except Exception as e:
        print(f"[CONFIG] ⚠️  Could not read data/favorites.json ({e}) — FAVORITES filter will match nothing")
        return set()


FAVORITE_TEAM_IDS = _load_favorite_team_ids()

# ── Odds cards (odds.py) ──────────────────────────────────────────────
# Posts pre-match 1X2 pricing as image cards, grouped 3-5 games per
# card, only for watched-team matches (same master list as everything
# else). Spread evenly across [ODDS_WINDOW_START_HOUR,
# ODDS_WINDOW_END_HOUR) UTC each day rather than dumped all at once —
# see bot.maybe_post_odds() for the scheduling. Needs ODDS_API_KEY
# (odds-api.io) — with no key set, this feature quietly does nothing.
POST_ODDS               = os.getenv("POST_ODDS", "true").lower() == "true"
ODDS_WINDOW_START_HOUR  = int(os.getenv("ODDS_WINDOW_START_HOUR", "5"))
ODDS_WINDOW_END_HOUR    = int(os.getenv("ODDS_WINDOW_END_HOUR", "10"))
ODDS_MIN_GAMES_PER_CARD = int(os.getenv("ODDS_MIN_GAMES_PER_CARD", "3"))
ODDS_MAX_GAMES_PER_CARD = int(os.getenv("ODDS_MAX_GAMES_PER_CARD", "5"))
