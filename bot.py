...
_odds_schedule:     dict             = {}   # today's odds-card schedule — see maybe_post_odds()
_post_timestamps:   list[float]      = []    # rolling timestamps for rate limiting
_last_post_time:    float            = 0.0   # for MIN_POST_GAP enforcement

# First-seen timestamp per goal (keyed by _key_goal), for the
# confirmation delay below. Deliberately in-memory only, not persisted
# to state_v2.json — a restart mid-confirmation-window just costs one
# extra poll of delay for that one goal (harmless, and consistent with
# "late is fine" policy), so it's not worth the extra state-file
# complexity of persisting it.
_goal_first_seen:  dict[str, float]  = {}


def _load_state():
    global _events, _last_preview_date, _pending_stale, _odds_schedule
    ...
    try:
        ...
        _odds_schedule          = raw.get("odds_schedule", {})
...
def _mark_goal_posted(mid: str, g: dict, idx: int = 0):
    ...
    for key in _goal_alias_keys(mid, g, idx):
        if key not in _events:
            _events[key] = now
    _goal_first_seen.pop(_key_goal(mid, g, idx), None)  # confirmation tracking no longer needed
    _save_state()


def _goal_confirmed(mid: str, g: dict, idx: int = 0) -> bool:
    """True once this goal has been seen on a poll at least
    config.GOAL_CONFIRM_SECONDS after the FIRST poll it was seen on —
    i.e. it has survived at least one extra look at SofaScore's
    incidents feed, not just the single tick it first appeared in.
    First call for a given goal always returns False (and starts its
    clock); every call after that checks the elapsed time. See the
    GOAL_CONFIRM_SECONDS comment in config.py for why this exists."""
    key = _key_goal(mid, g, idx)
    now = time.time()
    first_seen = _goal_first_seen.get(key)
    if first_seen is None:
        _goal_first_seen[key] = now
        return False
    return (now - first_seen) >= config.GOAL_CONFIRM_SECONDS
...
def maybe_post_preview():
    """Once a day, at config.DAILY_PREVIEW_HOUR (UTC), compiles the
    WHOLE day's watched-team fixtures — not just whatever happens to be
    live at that exact hour — via team_fixtures.get_today_schedule(),
    and posts them as one preview. get_today_schedule() fetches each
    watched team's upcoming fixtures fresh and filters to today's date,
    so this correctly includes matches that haven't kicked off yet even
    if the preview hour is early morning."""
    ...
    if now.hour != config.DAILY_PREVIEW_HOUR or _last_preview_date == today:
        return
    print("[BOT] 📅 Compiling today's full fixture list for the daily preview...")
    try:
        matches = team_fixtures.get_today_schedule()
    except Exception as e:
        print(f"[BOT] ⚠️  Could not build daily preview: {e}")
        return
...
def maybe_post_odds():
    """Posts pre-match 1X2 odds cards (odds.py), 3-5 games per card,
    spread evenly across [ODDS_WINDOW_START_HOUR, ODDS_WINDOW_END_HOUR)
    UTC. Today's matches are fetched and chunked into cards once — the
    first tick that lands inside the window each day — with each
    card's post time then spread evenly across the REST of the window
    rather than posted all at once. Safe to call every POLL_INTERVAL
    tick: a no-op outside the window, and outside the window build step
    it only ever renders/posts at most one card per tick (letting the
    normal MIN_POST_GAP/MAX_POSTS_PER_HOUR limits apply exactly like
    any other post, instead of bursting several odds cards back to
    back)."""
    global _odds_schedule
    if not config.POST_ODDS:
        return
    now   = datetime.now(timezone.utc)
    today = now.strftime("%Y-%m-%d")
    if not (config.ODDS_WINDOW_START_HOUR <= now.hour < config.ODDS_WINDOW_END_HOUR):
        return

    if _odds_schedule.get("date") != today:
        print("[BOT] 📊 Building today's odds card schedule...")
        try:
            matches = odds.get_todays_odds_matches()
        except Exception as e:
            print(f"[BOT] ⚠️  Could not fetch odds: {e}")
            matches = []
        chunks = odds.chunk_for_cards(matches, config.ODDS_MIN_GAMES_PER_CARD, config.ODDS_MAX_GAMES_PER_CARD)
        window_start = now.replace(hour=config.ODDS_WINDOW_START_HOUR, minute=0, second=0, microsecond=0)
        window_end   = now.replace(hour=config.ODDS_WINDOW_END_HOUR,   minute=0, second=0, microsecond=0)
        window_seconds = (window_end - window_start).total_seconds()
        n = len(chunks)
        cards = []
        for i, chunk in enumerate(chunks):
            # Evenly spaced across the window — the first card is never
            # held back waiting for its "turn", it can post as soon as
            # we're inside the window at all.
            offset = 0 if n <= 1 else (window_seconds * i / n)
            cards.append({
                "scheduled_ts": window_start.timestamp() + offset,
                "posted": False,
                "matches": chunk,
            })
        _odds_schedule = {"date": today, "cards": cards}
        _save_state()
        print(f"[BOT] 📊 {len(matches)} odds match(es) today, split into {n} card(s)")

    now_ts = now.timestamp()
    date_label = now.strftime("%B %d, %Y")
    total = len(_odds_schedule.get("cards", []))
    for i, card in enumerate(_odds_schedule.get("cards", [])):
        if card["posted"] or now_ts < card["scheduled_ts"]:
            continue
        key = f"odds:{today}:{i}"
        if _already_posted(key):
            card["posted"] = True
            continue
        print(f"[BOT] 📊 Posting odds card {i + 1}/{total} ({len(card['matches'])} game(s))...")
        img = _safe_image(
            graphics.render_odds_card, card["matches"], date_label,
            odds.ODDS_BOOKMAKER, i + 1, total,
        )
        if not img:
            print(f"[BOT] ⚠️  Odds card {i + 1}/{total} image failed to render — will retry next tick")
            break
        caption = poster.fmt_odds_caption(card["matches"], i + 1, total)
        if _post_if_new(key, caption, image_path=img):
            card["posted"] = True
            _save_state()
        break  # at most one odds card per tick — keeps normal rate limiting in charge of pacing
...
def process_match(match: dict):
    ...
    if (config.POST_GOALS and not match.get("_full_time_only")
        ...
        # Backup filter, on top of sofascore.py's _detect_var_disallowed_goals
        # (the primary fix — it catches a cancelled goal by noticing its
        # incident vanished from SofaScore's feed between polls, which is
        # reliable even once later goals bring the scoreline back in line
        # with the cancelled goal's old snapshot). This one stays as a
        # cheap second check for the case where a goal's snapshot is
        # simply inconsistent with the live "true" scoreboard field for
        # some other reason.
        ...
        for rank_pos, (idx, goal, key) in enumerate(ranked_keys):
            ...
            # Rule: don't post THIS goal live until it's survived one
            # extra look at the incidents feed (GOAL_CONFIRM_SECONDS) —
            # holds off (retried next tick), same as the ordering-hold
            # above. This is the primary defence against posting a goal
            # that's about to be VAR-overturned: waiting one poll cycle
            # is cheap, a false "GOAL" post is not. Deliberately checked
            # AFTER the ordering-hold above and BEFORE anything below can
            # mark this goal posted, so an unconfirmed goal behaves
            # exactly like an unposted one for every later-ranked goal's
            # own ordering check — nothing can ever jump ahead of it.
            if not _goal_confirmed(mid, goal, idx):
                continue
...
def main():
    ...
    print(f"  Odds cards    : {config.POST_ODDS} @ {config.ODDS_WINDOW_START_HOUR}:00-{config.ODDS_WINDOW_END_HOUR}:00 UTC"
          + (" (no ODDS_API_KEY set)" if config.POST_ODDS and not odds.ODDS_API_KEY else ""))
