...
def fmt_odds_caption(matches: list, card_index: int = 1, total_cards: int = 1) -> str:
    """Short Facebook caption for an odds card image — all the actual
    numbers live in the image itself (graphics.render_odds_card), so
    this is deliberately brief: a header line, an entertainment-only
    disclaimer (never framed as a tip/pick), and up to 3 hashtags."""
    header = "Today's Odds"
    if total_cards > 1:
        header += f" ({card_index}/{total_cards})"
    body = ["Prices for entertainment only — always check the bookmaker before betting."]
    teams = []
    for m in matches[:2]:
        teams.append(m["home"].replace(" ", ""))
        teams.append(m["away"].replace(" ", ""))
    tags = (["Odds"] + teams)[:3]
    return _build_post(header, body, tags, marker="📊")
