...
def render_scoreboard_card(kind: str, home_name: str, away_name: str,
    ...
    # Row: team name — hexagon score — team name. No crest/logo images
    # anymore (team-logo fetches were unreliable on Railway's cloud
    # IPs, so cards would randomly show a broken/blank badge or a
    # generic initials hexagon). Text-only is guaranteed to render
    # every time, so the team name itself sits inline with the score
    # hexagon instead — home name left-aligned to the margin, away
    # name right-aligned to the margin, both vertically centered on
    # the hexagon's row.
    row_cy = 420
    hex_w, hex_h = 560, 270
    side_margin = 56
    # Leave real clearance between the text block and the hexagon so
    # a long name can never crowd/overlap the score.
    name_max_w = int(W / 2 - hex_w / 2 - side_margin - 24)

    draw = _draw_score_hexagon(img, draw, W / 2, row_cy, home_score, away_score,
                                accent, w=hex_w, h=hex_h)

    def _fit_name_font(name: str, max_w: int):
        """Largest extrabold size (40 down to 22) that fits `name` on
        one line within max_w; below that, wrap onto two lines at the
        smallest size instead of shrinking further."""
        for size in (40, 36, 32, 28, 24):
            font = _font(size, "extrabold")
            if _text_w(draw, name, font) <= max_w:
                return font, [name]
        font = _font(24, "extrabold")
        lines = textwrap.wrap(name, width=14, break_long_words=False,
                               break_on_hyphens=False)[:2] or [name]
        return font, lines

    name_lines_drawn = 0
    for name, align in ((home_name, "left"), (away_name, "right")):
        font, lines = _fit_name_font(name.upper(), name_max_w)
        ...
        for i, line in enumerate(lines):
            ...
            lx = side_margin if align == "left" else W - side_margin - lw
            draw.text((lx, ty + i * line_h), line, font=font, fill="#FFFFFF")
    ...
    if home_penalty is not None or away_penalty is not None:
        ...
        pens_y = below_row_y
        for pens, align, is_winner in (
            (home_penalty, "left", pens_won_home),
            (away_penalty, "right", not pens_won_home),
        ):
            if pens is None:
                continue
            ...
            lx = side_margin if align == "left" else W - side_margin - lw
            draw.text((lx, pens_y), label, font=pens_label_font,
                      fill=accent if is_winner else "#8B94B8")
            num_text = str(pens)
            nw = _text_w(draw, num_text, pens_num_font)
            nx = side_margin if align == "left" else W - side_margin - nw
            draw.text((nx, pens_y + 26), num_text, font=pens_num_font,
                      fill=accent if is_winner else "#8B94B8")
        pens_block_h = 26 + 66

    # Event line(s) below the name/hexagon row. Side-anchored lines
    # (goal scorer, full-time scorer list, red card) sit on the
    # scoring team's own side — left-aligned under the home name,
    # right-aligned under the away name — so it's immediately clear
    # whose event it is, rather than a single line straddling the
    # middle of the card that reads ambiguous when both teams scored.
    side_y_start = below_row_y + pens_block_h
    side_font = _font(26, "semibold")

    def _draw_side_lines(text: str, align: str):
        ...
        for i, raw_line in enumerate(raw_lines):
            ...
            if y + len(wrapped) * line_h > safe_bottom:
                ...
                lx = side_margin if align == "left" else W - side_margin - lw
                draw.text((lx, y), more_text, font=side_font, fill="#FFFFFF")
                return
            for line in wrapped:
                lw = _text_w(draw, line, side_font)
                lx = side_margin if align == "left" else W - side_margin - lw
                draw.text((lx, y), line, font=side_font, fill="#FFFFFF")
                y += line_h

    if home_event_line:
        _draw_side_lines(home_event_line, "left")
    if away_event_line:
        _draw_side_lines(away_event_line, "right")
