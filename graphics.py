...
SCOREBOARD_NAVY_TOP    = "#182569"
SCOREBOARD_NAVY_BOTTOM = "#0F1A4A"
SCOREBOARD_INK         = "#0B1E4D"   # score text color (dark navy on the white pill)
SCOREBOARD_TEXT        = "#FFFFFF"   # main text color — white on the navy card bg
SCOREBOARD_SUBTEXT     = "#C7D0EE"   # muted light blue-white for competition/tagline lines
SCOREBOARD_PILL        = "#FFFFFF"   # score pill is always white now, not per-kind accent
...
_LIVE_BADGE_CACHE = None

def _live_badge(target_w: int = 233):
    """Loads the transparent LIVE badge PNG once and caches it, scaled
    to the size measured off the reference template (~233px wide on
    the 1024px card, badge's own aspect ratio preserved). The source
    file has transparent padding around the actual pill graphic, so
    it's cropped to its real content bounding box first — otherwise
    the padding shifts the visible badge away from the paste point."""
    global _LIVE_BADGE_CACHE
    if _LIVE_BADGE_CACHE is None:
        path = os.path.join(ASSETS_DIR, "live_badge.png")
        badge = Image.open(path).convert("RGBA")
        badge = badge.crop(badge.getbbox())
        bw, bh = badge.size
        target_h = int(bh * (target_w / bw))
        _LIVE_BADGE_CACHE = badge.resize((target_w, target_h), Image.LANCZOS)
    return _LIVE_BADGE_CACHE


def _draw_live_badge(img):
    """Pastes the LIVE badge top-left, matching the reference card's
    position (left edge ~44px, top ~50px on the 1024px canvas)."""
    badge = _live_badge()
    img.paste(badge, (44, 50), badge)
...
def _draw_score_pill(img, draw, cx: float, cy: float, home_score, away_score,
    ...
    ldraw = ImageDraw.Draw(layer)
    radius = int(h * 0.30)
    ldraw.rounded_rectangle([0, 0, w - 1, h - 1], radius=radius, fill=_hex_to_rgb(SCOREBOARD_PILL))
...
def render_scoreboard_card(kind: str, home_name: str, away_name: str,
    ...
    draw.text((W / 2 - hw / 2, 68), header_text, font=header_font, fill=SCOREBOARD_TEXT)
    ...
    if competition:
        ...
        draw.text((W / 2 - cw / 2, comp_y), comp_text, font=comp_font, fill=SCOREBOARD_SUBTEXT)
    ...
    for name, cx in ((home_name, home_cx), (away_name, away_cx)):
        ...
        for i, line in enumerate(lines):
            lw = _text_w(draw, line, name_font)
            draw.text((cx - lw / 2, ty + i * name_line_h), line, font=name_font, fill=SCOREBOARD_TEXT)
    ...
    def _draw_side_lines(text: str, cx: float):
        ...
        for i, raw_line in enumerate(raw_lines):
            ...
            if y + len(wrapped) * line_h > safe_bottom:
                ...
                draw.text((cx - lw / 2, y), more_text, font=side_font, fill=SCOREBOARD_TEXT)
                return
            for line in wrapped:
                lw = _text_w(draw, line, side_font)
                draw.text((cx - lw / 2, y), line, font=side_font, fill=SCOREBOARD_TEXT)
                y += line_h
    ...
    if event_line and not (home_event_line or away_event_line):
        ...
        for raw_line in event_line.split("\n"):
            for line in textwrap.wrap(raw_line, width=32) or [""]:
                lw = _text_w(draw, line, ev_font)
                draw.text((W / 2 - lw / 2, y), line, font=ev_font, fill=SCOREBOARD_TEXT)
                y += 44
...
def render_motm_card(player_name: str, team_name: str, rating,
    ...
    top_line = SCOREBOARD_HEADERS.get("motm", SCOREBOARD_HEADERS["default"])
    header_font = _font(50, "extrabold")
    hw = _text_w(draw, top_line, header_font)
    draw.text((W / 2 - hw / 2, 64), top_line, font=header_font, fill=SCOREBOARD_TEXT)

    if competition:
        comp_font = _font(24, "regular")
        comp_text = competition.upper()
        cw = _text_w(draw, comp_text, comp_font)
        draw.text((W / 2 - cw / 2, 134), comp_text, font=comp_font, fill=SCOREBOARD_SUBTEXT)
    ...
    for line in name_lines:
        lw = _text_w(draw, line, name_font)
        draw.text((W / 2 - lw / 2, ny), line, font=name_font, fill=SCOREBOARD_TEXT)
        ny += 54
    ...
    if rating is not None:
        ...
        draw.text((W / 2 - rw / 2, ny + 14), rating_text, font=rating_font, fill=SCOREBOARD_SUBTEXT)
...
def _draw_brand_ribbon_v2(img, draw, accent_hex: str):
    ...
    draw = ImageDraw.Draw(img)
    draw.text((44, H - ribbon_h + 14), BRAND_NAME, font=_font(36, "extrabold"), fill=SCOREBOARD_TEXT)
    draw.text((44, H - ribbon_h + 56), BRAND_TAGLINE, font=_font(25, "regular"), fill=SCOREBOARD_SUBTEXT)
    return draw
